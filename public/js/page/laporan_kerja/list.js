...
$(() => {
    table = $("#table-data").DataTable({
        ...
        processing: true,
        ajax: {
            url: BASE_URL + "laporan-kerja/data",
            type: "get",
            dataType: "json",
        },
        columns: [
    ...
    $(".btn-tambah").on("click", function () {
        $("#form-laporan-kerja")[0].reset();
        $("#id").val("");
        $('input[type="file"]').val("");
        $("#existing-pdf").html("");
        $("#pdf").attr("required", true);
        $("#modal-laporan-kerja").modal("show");
    });
    ...
    $("#table-data").on("click", ".btn-update", function () {
        ...
        $.get(BASE_URL + "laporan-kerja/edit/" + id, (res) => {
            if (res.status) {
                let data = res.data;
                $("#id").val(data.id);
    ...
    $("#form-laporan-kerja").on("submit", function (e) {
        e.preventDefault();
        let id = $("#id").val();
        let url = id
            ? BASE_URL + "laporan-kerja/update"
            : BASE_URL + "laporan-kerja/store";
        ...
        $.ajax({
            ...
            contentType: false,
            beforeSend: () => {
                $("#modal-laporan-kerja .modal-content").LoadingOverlay("show");
                $("#form-laporan-kerja button[type='submit']").attr("disabled", true);
            },
            success: (res) => {
                if (res.status) {
                    App.showToastr.success("Sukses", res.message);
                    $("#modal-laporan-kerja").modal("hide");
                    table.ajax.reload();
                }
            },
            error: (err) => {
            ...
            complete: () => {
                $("#modal-laporan-kerja .modal-content").LoadingOverlay("hide");
                $("#form-laporan-kerja button[type='submit']").attr("disabled", false);
            },
        });
    });
