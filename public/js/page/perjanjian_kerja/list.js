...
$(() => {
    table = $("#table-data").DataTable({
        ...
        processing: true,
        ajax: {
            url: BASE_URL + "perjanjian-kerja/data",
            type: "get",
            dataType: "json",
        },
        columns: [
    ...
    $(".btn-tambah").on("click", function () {
        $("#form-perjanjian-kerja")[0].reset();
        $("#id").val("");
        $('input[type="file"]').val("");
        $("#existing-pdf").html("");
        $("#pdf").attr("required", true);
        $("#modal-perjanjian-kerja").modal("show");
    });
    ...
    $("#table-data").on("click", ".btn-update", function () {
        ...
        $.get(BASE_URL + "perjanjian-kerja/edit/" + id, (res) => {
            if (res.status) {
                let data = res.data;
                $("#id").val(data.id);
    ...
    $("#form-perjanjian-kerja").on("submit", function (e) {
        e.preventDefault();
        let id = $("#id").val();
        let url = id
            ? BASE_URL + "perjanjian-kerja/update"
            : BASE_URL + "perjanjian-kerja/store";
        ...
        $.ajax({
            ...
            contentType: false,
            beforeSend: () => {
                $("#modal-perjanjian-kerja .modal-content").LoadingOverlay("show");
                $("#form-perjanjian-kerja button[type='submit']").attr("disabled", true);
            },
            success: (res) => {
                if (res.status) {
                    App.showToastr.success("Sukses", res.message);
                    $("#modal-perjanjian-kerja").modal("hide");
                    table.ajax.reload();
                }
            },
            error: (err) => {
            ...
            complete: () => {
                $("#modal-perjanjian-kerja .modal-content").LoadingOverlay("hide");
                $("#form-perjanjian-kerja button[type='submit']").attr("disabled", false);
            },
        });
    });
    ...
    $("#table-data").on("click", ".btn-delete", function () {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.post(
                    BASE_URL + "perjanjian-kerja/delete",
