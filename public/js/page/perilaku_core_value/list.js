...
$(() => {
    table = $("#table-data").DataTable({
        ...
        processing: true,
        ajax: {
            url: BASE_URL + "perilaku-core-value/data",
            type: "get",
            dataType: "json",
        },
        columns: [
    ...
    $(".btn-tambah").on("click", function () {
        $("#form-perilaku-core-value")[0].reset();
        $("#id").val("");
        $('input[type="file"]').val("");
        $("#existing-pdf").html("");
        $("#pdf").attr("required", true);
        $("#modal-perilaku-core-value").modal("show");
    });
    ...
    $("#table-data").on("click", ".btn-update", function () {
        ...
        $.get(BASE_URL + "perilaku-core-value/edit/" + id, (res) => {
            if (res.status) {
                let data = res.data;
                $("#id").val(data.id);
    ...
    $("#form-perilaku-core-value").on("submit", function (e) {
        e.preventDefault();
        let id = $("#id").val();
        let url = id
            ? BASE_URL + "perilaku-core-value/update"
            : BASE_URL + "perilaku-core-value/store";
        ...
        $.ajax({
            ...
            contentType: false,
            beforeSend: () => {
                $("#modal-perilaku-core-value .modal-content").LoadingOverlay("show");
                $("#form-perilaku-core-value button[type='submit']").attr("disabled", true);
            },
            success: (res) => {
                if (res.status) {
                    App.showToastr.success("Sukses", res.message);
                    $("#modal-perilaku-core-value").modal("hide");
                    table.ajax.reload();
                }
            },
            error: (err) => {
            ...
            complete: () => {
                $("#modal-perilaku-core-value .modal-content").LoadingOverlay("hide");
                $("#form-perilaku-core-value button[type='submit']").attr("disabled", false);
            },
        });
    });
    ...
    $("#table-data").on("click", ".btn-delete", function () {
        let id = $(this).data("id");
        Swal.fire({
            title: "Hapus Perilaku & Core Value?",
            ...
            if (result.isConfirmed) {
                $.post(
                    BASE_URL + "perilaku-core-value/delete",
