// public/js/page/ppid/informasi_dikecualikan/list.js
let table;
...
$(() => {
    table = $("#table-data").DataTable({
        ...
        processing: true,
        ajax: {
            url: BASE_URL + "ppid/informasi-dikecualikan/data",
            type: "get",
            dataType: "json",
        },
        columns: [
    ...
    $(".btn-tambah").on("click", function () {
        $("#form-informasi-dikecualikan")[0].reset();
        $("#id").val("");
        $('input[name="file"]').val("").attr("required", true);
        $("#existing-file").html("");
        $("#modal-informasi-dikecualikan").modal("show");
    });
    ...
    $("#table-data").on("click", ".btn-update", function () {
        ...
        $.get(BASE_URL + "ppid/informasi-dikecualikan/edit/" + id, (res) => {
            if (res.status) {
                let data = res.data;
                $("#id").val(data.id);
    ...
    $("#form-informasi-dikecualikan").on("submit", function (e) {
        e.preventDefault();
        let id = $("#id").val();
        let url = id ? BASE_URL + "ppid/informasi-dikecualikan/update" : BASE_URL + "ppid/informasi-dikecualikan/store";
        ...
        $.ajax({
            ...
            contentType: false,
            beforeSend: () => {
                $("#modal-informasi-dikecualikan .modal-content").LoadingOverlay("show");
                $("#form-informasi-dikecualikan button[type='submit']").attr("disabled", true);
            },
            success: (res) => {
                if (res.status) {
                    App.showToastr.success("Sukses", res.message);
                    $("#modal-informasi-dikecualikan").modal("hide");
                    table.ajax.reload();
                }
            },
            error: (err) => {
            ...
            complete: () => {
                $("#modal-informasi-dikecualikan .modal-content").LoadingOverlay("hide");
                $("#form-informasi-dikecualikan button[type='submit']").attr("disabled", false);
            },
        });
    });
    ...
    $("#table-data").on("click", ".btn-delete", function () {
        let id = $(this).data("id");
        Swal.fire({
            title: "Hapus Informasi Dikecualikan?",
            ...
            if (result.isConfirmed) {
                $.post(
                    BASE_URL + "ppid/informasi-dikecualikan/delete",
