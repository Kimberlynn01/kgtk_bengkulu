// public/js/page/ppid/daftar_informasi_publik/list.js
let table;
...
$(() => {
    table = $("#table-data").DataTable({
        ...
        processing: true,
        ajax: {
            url: BASE_URL + "ppid/daftar-informasi-publik/data",
            type: "get",
            dataType: "json",
        },
        columns: [
    ...
    $(".btn-tambah").on("click", function () {
        $("#form-daftar-informasi-publik")[0].reset();
        $("#id").val("");
        $('input[name="file"]').val("").attr("required", true);
        $("#existing-file").html("");
        $("#modal-daftar-informasi-publik").modal("show");
    });
    ...
    $("#table-data").on("click", ".btn-update", function () {
        ...
        $.get(BASE_URL + "ppid/daftar-informasi-publik/edit/" + id, (res) => {
            if (res.status) {
                let data = res.data;
                $("#id").val(data.id);
    ...
    $("#form-daftar-informasi-publik").on("submit", function (e) {
        e.preventDefault();
        let id = $("#id").val();
        let url = id ? BASE_URL + "ppid/daftar-informasi-publik/update" : BASE_URL + "ppid/daftar-informasi-publik/store";
        ...
        $.ajax({
            ...
            contentType: false,
            beforeSend: () => {
                $("#modal-daftar-informasi-publik .modal-content").LoadingOverlay("show");
                $("#form-daftar-informasi-publik button[type='submit']").attr("disabled", true);
            },
            success: (res) => {
                if (res.status) {
                    App.showToastr.success("Sukses", res.message);
                    $("#modal-daftar-informasi-publik").modal("hide");
                    table.ajax.reload();
                }
            },
            error: (err) => {
            ...
            complete: () => {
                $("#modal-daftar-informasi-publik .modal-content").LoadingOverlay("hide");
                $("#form-daftar-informasi-publik button[type='submit']").attr("disabled", false);
            },
        });
    });
    ...
    $("#table-data").on("click", ".btn-delete", function () {
        let id = $(this).data("id");
        Swal.fire({
            title: "Hapus Daftar Informasi Publik?",
            ...
            if (result.isConfirmed) {
                $.post(
                    BASE_URL + "ppid/daftar-informasi-publik/delete",
